...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def saveAllObjects(self):
        ...
        new_path = QtWidgets.QFileDialog.getExistingDirectory(self, "Select where to export objects")

        if new_path is '':
            return

        text, ok = QInputDialog.getText(self, 'Folder Name Setter', 'Enter Folder Name of Objects')
        ...
        if not os.path.isdir(new_path + "/" + NewName):
            os.mkdir(new_path + "/" + NewName)

        curr_path = new_path+"/"+NewName
        ...
        for object in objects:
            ...
            for i in range(len(object.tiles)):
                for tile in object.tiles[i]:
                    if (Tileset.slot == 0) or ((tile[2] & 3) != 0):
                        painter.drawPixmap(Xoffset, Yoffset, tiles[tile[1]].image)
                    Tilebuffer += (tiles[tile[1]].byte0).to_bytes(1, 'big')
                    Tilebuffer += (tiles[tile[1]].byte1).to_bytes(1, 'big')
                    Tilebuffer += (tiles[tile[1]].byte2).to_bytes(1, 'big')
                    Tilebuffer += (tiles[tile[1]].byte3).to_bytes(1, 'big')
                    Tilebuffer += (tiles[tile[1]].byte4).to_bytes(1, 'big')
                    Tilebuffer += (tiles[tile[1]].byte5).to_bytes(1, 'big')
                    Tilebuffer += (tiles[tile[1]].byte6).to_bytes(1, 'big')
                    Tilebuffer += (tiles[tile[1]].byte7).to_bytes(1, 'big')
                    Xoffset += 60
                Xoffset = 0
                Yoffset += 60
            ...
            print(curr_path + "/object_" + str(count) + ".png")

            tex.save(curr_path + "/object_" + str(count) + ".png", "PNG")

            with open(curr_path + "/object_" + str(count) + ".colls", "wb+") as colls:
                colls.write(Tilebuffer)

            with open(curr_path + "/object_" + str(count) + ".objlyt", "wb+") as objlyt:
                objlyt.write(Objbuffer)

            with open(curr_path + "/object_" + str(count) + ".meta", "wb+") as meta:
                meta.write(Metabuffer)

            objmodel = QtGui.QStandardItemModel()

            objmodel.appendRow(QtGui.QStandardItem(QtGui.QIcon(tex), 'Object {0}'.format(count)))
        ...
        for object in objects:
            ...
            for i in range(len(object.tiles)):
                for tile in object.tiles[i]:
                    if (Tileset.slot == 0) or ((tile[2] & 3) != 0):
                        painter.drawPixmap(Xoffset, Yoffset, tiles[tile[1]].normalmap)
                    Xoffset += 60
                Xoffset = 0
                Yoffset += 60
            ...
            print(curr_path + "/object_" + str(count) + "_nml.png")

            tex.save(curr_path + "/object_" + str(count) + "_nml.png", "PNG")
    ...
    def saveObject(self):
        ...
        maxObjects = len(objects)-1

        print('')

        old_path = curr_path

        new_path = QtWidgets.QFileDialog.getExistingDirectory(self, "Select where to export object")

        if new_path is '':
            return

        text, ok = QInputDialog.getText(self, 'Folder Name Setter', 'Enter Folder Name of Object')

        NewName = "Objects"

        num ,oks = QInputDialog.getInt(self,"Object Exporter","Enter Object Number To Export", 0, 0, maxObjects)
    ...
    def setupMenus(self):
        ...
        fileMenu.addAction("Dump All Objects...", self.saveAllObjects, QtGui.QKeySequence('Ctrl+Shift+A'))
        fileMenu.addAction("Dump Object...", self.saveObject, QtGui.QKeySequence('Ctrl+Shift+O'))
